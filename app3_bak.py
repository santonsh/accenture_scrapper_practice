...
def get_queue_logger(loggerQueue):
        """this simply returns queue logger based on the provided queue"""
...
def job_enqueuer(p_id, exitFlag, loggerQueue, urlResultQueue, jobQueue, jobQueueSize, urlDict):
    logger = get_queue_logger(loggerQueue)
...
def page_downloader_and_parser(p_id, exitFlag, loggerQueue, jobQueue, jobQueueSize, urlResultQueue, emailStoreQueue, urlDict, emailDict, locality):
    logger = get_queue_logger(loggerQueue)
    process_exitFlag = False
    fail_cnt = 0
    ...
    while exitFlag.value == 0 and process_exitFlag == False:
        ...
        try:
            ...
            try:
                [base_url, page] = get_html_page(loggerQueue, url)
                if page == '':
...
def get_html_page(loggerQueue, url):
...
class Scrapper():
    def __init__(self, loggerQueue, processes_number=2, test_graph={}, locality='any'):
    ...
    def start(self):
        ...
        p = Process(target=watch_dog, args=(
            'watch_dog_0', self.exitFlag, self.loggerQueue, self.taskQueueSize, self.collected_emails_dict, self.collected_urls_dict))
        ...
        for i in range(self.processes_number):
            ...
            p = Process(target=page_downloader_and_parser , args=(
                p_id, self.exitFlag, self.loggerQueue, self.taskQueue, self.taskQueueSize, self.urlResultQueue, self.emailStoreQueue, self.collected_urls_dict, self.collected_emails_dict, self.locality))
            self.processes.append(p)
            p.start()
...
if __name__ == '__main__':
    ...
    log_queue = Queue(-1)
    formatter = logging.Formatter('%(relativeCreated)d: %(levelname)s: %(threadName)s: %(message)s')
    ...
    if args.test:
        ...
        s = Scrapper(log_queue, processes_number=8, locality='subdomain')
        s.set_urls(urls)
