...
if __name__ == '__main__':
    ...
    if args.test == True:
        ...
        print(len(set(found_urls)-set(test_urls)))
        print(len(set(test_urls)-set(found_urls)))

        not_found = set(test_urls)-set(found_urls)
        if len(not_found)>0:
            print('Test run finished. Not_found urls:')
        for u in not_found:
            print(u)
        if len(not_found) == 0:
